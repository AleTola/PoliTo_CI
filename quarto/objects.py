...
class Quarto(object):
    ...
    def reset(self):
        # creates the array of the shape (BOARD_SIDE, BOARD_SIDE)
        self._board = np.ones(shape=(self.BOARD_SIDE, self.BOARD_SIDE), dtype=int) * -1

        self._binary_board = np.full(shape=(self.BOARD_SIDE, self.BOARD_SIDE, 4), fill_value=np.nan)
